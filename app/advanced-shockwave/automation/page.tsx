...
export default function ApexAutomationPage() {
  ...
  async function loadRule() {
    try {
      const response = await fetch('/api/advanced-shockwave/rules/quiz-lead-boost')
      if (response.ok) {
  ...
  async function loadExecutions() {
    try {
      const response = await fetch('/api/advanced-shockwave/rules/quiz-lead-boost/executions?limit=20')
      if (response.ok) {
        const data = await response.json()
        setExecutions(data)
  ...
  async function loadLastRun() {
    try {
      ...
      const response = await fetch('/api/advanced-shockwave/rules/quiz-lead-boost/executions?limit=1&any_status=true')
      if (response.ok) {
        const data = await response.json()
        if (data && data.length > 0) {
          setLastRun(data[0].created_at)
  ...
  async function loadMonitoredAdSets() {
    ...
    try {
      const response = await fetch('/api/advanced-shockwave/rules/quiz-lead-boost/monitored-adsets')
      const data = await response.json()
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <h1>Advanced Shockwave Pain Relief — Automation Rules</h1>
